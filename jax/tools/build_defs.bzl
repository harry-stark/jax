...
def jax_to_hlo(name, deps, fn, input_shapes, constants = None):
    jax_to_proto(name, deps, fn, input_shapes, constants = constants, format = "HLO")

def jax_to_tf(name, deps, fn, input_shapes, constants = None):
    jax_to_proto(name, deps, fn, input_shapes, constants = constants, format = "TF")

def jax_to_proto(name, deps, fn, input_shapes, constants = None, format = "HLO"):
    """Creates a genrule that uses jax_to_proto.py to make proto from a JAX func.
    ...
    Then we can invoke this macro as follows.
      ...
      $ cat your/thing/BUILD
      load("//jax/tools:build_defs.bzl", "jax_to_proto")
    ...
    if format == "TF":
        deps.append("//third_party/py/tensorflow")
        deps.append("//third_party/py/jax/experimental/jax2tf")

    # Our goal here is to create a py_binary which depends on `deps` and
    # invokes the main function defined in jax_to_proto.py.
    #
    # At first blush it seems that we can do this in a straightforward way:
    #
    #   native.py_binary(main = "jax_to_proto.py").
    #
    # The problem with this is that this py_binary lives in the user's package,
    # whereas jax_to_proto.py lives inside JAX's package.  Bazel delivers a stern
    ...
    runner = name + "_jax_to_proto_main"
    native.genrule(
        name = runner + "_gen",
        outs = [runner + ".py"],
        cmd = """cat <<EOF > '$(location {runner}.py)'
from absl import app
import jax.tools.jax_to_proto as jax_to_proto

jax_to_proto.set_up_flags()
app.run(jax_to_proto.main)
EOF
    ...
    native.py_binary(
        ...
        deps = deps + [
            ...
            "//third_party/py/jax/tools:jax_to_proto",
        ],
    )
    ...
    native.genrule(
        name = name + "_jax_to_proto_genrule",
        ...
        '$(location {runner})' \
          ...
          --proto_format {format} \
          --proto_dest '$(location {name}.pb)' \
          --text_dest '$(location {name}.txt)' \
        """.format(
            name = name,
            fn = fn,
