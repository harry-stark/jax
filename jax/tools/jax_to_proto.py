...
Usage:
  ...
  $ python jax_to_proto.py \
    --fn prog.fn \
    --input_shapes '[("y": "f32[128,32]"), ("x", "f32[8,128]")]' \
    --constants '{"z": 3.14159}' \
    --proto_format HLO \
    --text_dest /tmp/fn_hlo.txt \
    --proto_dest /tmp/fn_hlo.pb
...
def jax_to_proto(fn, input_shapes, *, constants=None, format):
  """Converts a JAX function to a serialized proto and a debug txt dump.
  ...
  Returns:
    A tuple (serialized_proto, debug_text).
  """
  if not constants:
    constants = {}
...
jax_to_hlo = functools.partial(jax_to_proto, format='HLO')
jax_to_tf = functools.partial(jax_to_proto, format='TF')
...
def main(argv):
  ...
  if not FLAGS.proto_dest and not FLAGS.text_dest:
    raise app.Error('At least one of --proto_dest and '
                    '--text_dest is required.')
  ...
  proto, text = jax_to_proto(fn, input_shapes, constants=constants,
                             format=FLAGS.proto_format)

  if FLAGS.proto_dest:
    with open(FLAGS.proto_dest, 'wb') as f:
      f.write(proto)

  if FLAGS.text_dest:
    with open(FLAGS.text_dest, 'w') as f:
      f.write(text)
...
def set_up_flags():
  ...
  flags.DEFINE_enum('proto_format', 'HLO', ('HLO', 'TF'), 'Output format.')
  flags.DEFINE_string('proto_dest', None, 'File to write HLO proto')
  flags.DEFINE_string('text_dest', None, 'File to write HLO text')
