...
from jax.tools import jax_to_proto
...
class JaxToProtoTest(absltest.TestCase):

  def test_jax_to_hlo_axpy(self):
    hlo_proto, hlo_text = jax_to_proto.jax_to_hlo(axpy, [
        ('y', jax_to_proto.parse_shape_str('f32[128]')),
        ('a', jax_to_proto.parse_shape_str('f32[]')),
        ('x', jax_to_proto.parse_shape_str('f32[128,2]')),
    ])
  ...
  def test_jax_to_hlo_with_constants(self):
    ...
    _, hlo_text = jax_to_proto.jax_to_hlo(
        fn,
        input_shapes=[
            ('x', jax_to_proto.parse_shape_str('f32[128]')),
            ('y', jax_to_proto.parse_shape_str('f32[128]')),
        ],
        constants={
            'a': 123456,
  ...
  def test_parse_shape_str_invalid(self):
    with self.assertRaisesRegex(ValueError, 'Invalid shape.*foo'):
      jax_to_proto.parse_shape_str('foo[]')

  def test_jax_to_tf_axpy(self):
    tf_proto, tf_text = jax_to_proto.jax_to_tf(axpy, [
        ('y', jax_to_proto.parse_shape_str('f32[128]')),
        ('a', jax_to_proto.parse_shape_str('f32[]')),
        ('x', jax_to_proto.parse_shape_str('f32[128,2]')),
    ])
  ...
  def assertIdentityOp(self, op: tf.Operation, expected_shape, expected_dtype):
  ...
  def assertParsedShape(self, s: str, expected_shape, expected_dtype):
    p = jax_to_proto.parse_shape_str(s)
